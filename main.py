...
def go(config: DictConfig):
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        if "data_split" in active_steps:
            mlflow.run(
            f"{config['main']['components_repository']}/train_val_test_split",'main', #course provided
            parameters = {
                'input': 'clean_sample.csv:latest', #input csv file
        ...
        if "train_random_forest" in active_steps:
            ...
            entry_point = 'main',
            parameters={
                'trainval_artifact': 'trainval_data.csv:latest', #train dataset
                'val_size': config['modeling']['val_size'],
                'random_seed': config['modeling']['random_seed'],
                'stratify_by': config['modeling']['stratify_by'],
                'rf_config': rf_config,
                'max_tfidf_features': config['modeling']['max_tfidf_features'],
                'output_artifact': 'random_forest_export' #output file
                }
            )
